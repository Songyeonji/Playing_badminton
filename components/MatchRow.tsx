  ...
  return (
    <div className="bg-surface rounded-xl shadow-lg border-2 border-accent p-6 mb-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-2">
              <input type="text" value={teamA1} onChange={(e) => setTeamA1(e.target.value)} placeholder="선수 1" className="w-28 px-3 py-3 border-2 border-teal rounded-lg focus:outline-none focus:border-darkTeal bg-white text-gray-900 font-medium text-base placeholder-gray-400" />
              <input type="text" value={teamA2} onChange={(e) => setTeamA2(e.target.value)} placeholder="선수 2" className="w-28 px-3 py-3 border-2 border-teal rounded-lg focus:outline-none focus:border-darkTeal bg-white text-gray-900 font-medium text-base placeholder-gray-400" />
            </div>
          </div>
          <div className="flex flex-col items-center gap-2 mt-6">
            <div className="px-5 py-3 bg-darkTeal text-white font-bold rounded-lg shadow-md text-lg">VS</div>
            <div className="flex items-center gap-2">
              <input type="text" inputMode="numeric" pattern="[0-9]*" value={totalScoreA} onChange={(e) => setTotalScoreA(e.target.value)} onBlur={validateOnBlur} placeholder="0" className="w-16 px-2 py-2 border-2 border-teal rounded text-center font-bold bg-white text-gray-900 text-base" />
              <span className="text-xl font-bold text-gray-600">:</span>
              <input type="text" inputMode="numeric" pattern="[0-9]*" value={totalScoreB} onChange={(e) => setTotalScoreB(e.target.value)} onBlur={validateOnBlur} placeholder="0" className="w-16 px-2 py-2 border-2 border-accent rounded text-center font-bold bg-white text-gray-900 text-base" />
            </div>
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-sm font-bold text-gray-800">{settings.teamBName}</label>
            <div className="flex gap-2">
              <input type="text" value={teamB1} onChange={(e) => setTeamB1(e.target.value)} placeholder="선수 3" className="w-28 px-3 py-3 border-2 border-accent rounded-lg focus:outline-none focus:border-teal bg-white text-gray-900 font-medium text-base placeholder-gray-400" />
              <input type="text" value={teamB2} onChange={(e) => setTeamB2(e.target.value)} placeholder="선수 4" className="w-28 px-3 py-3 border-2 border-accent rounded-lg focus:outline-none focus:border-teal bg-white text-gray-900 font-medium text-base placeholder-gray-400" />
            </div>
          </div>
        </div>
      </div>
        ...
        <label className="flex items-center justify-between text-base font-bold text-gray-900 mb-3">
          ...
          {sets.length < 5 && (
            <div className="flex flex-col items-end gap-1">
              <button onClick={handleAddSet} className="flex items-center gap-1 text-sm text-gray-800 hover:text-gray-900 font-bold bg-accent px-3 py-2 rounded-lg hover:bg-opacity-80 transition-colors">
          ...
          {sets.map((set, index) => (
            <div key={index} className="flex items-center gap-3 bg-white p-4 rounded-lg border-2 border-accent shadow-sm">
              ...
              <div className="flex items-center gap-3">
                <input type="text" inputMode="numeric" pattern="[0-9]*" value={set.a} onChange={(e) => handleSetChange(index, 'a', e.target.value)} className="w-20 px-4 py-3 border-2 border-teal rounded-lg focus:outline-none focus:border-darkTeal text-center font-bold bg-white text-gray-900 text-lg" />
                <span className="text-3xl font-bold text-gray-600">:</span>
                <input type="text" inputMode="numeric" pattern="[0-9]*" value={set.b} onChange={(e) => handleSetChange(index, 'b', e.target.value)} className="w-20 px-4 py-3 border-2 border-accent rounded-lg focus:outline-none focus:border-teal text-center font-bold bg-white text-gray-900 text-lg" />
              </div>
              {sets.length > 1 && (
      ...
      <div className="flex gap-3">
        <button onClick={handleAddMatch} className="flex items-center gap-2 px-6 py-3 bg-teal text-white rounded-lg hover:bg-opacity-90 transition-all shadow-md hover:shadow-lg font-bold text-base">
