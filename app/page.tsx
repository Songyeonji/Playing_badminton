      ...
      <div className="bg-cyan-600 shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
          <div className="flex items-center gap-2 sm:gap-3">
            <GiShuttlecock className="text-3xl sm:text-4xl text-white" />
            <div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 pb-24 sm:pb-28">
        ...
        <div className="mb-6 sm:mb-8">
          <button
            ...
            className="w-full sm:w-auto flex items-center justify-center gap-2 sm:gap-3 px-6 py-3 sm:px-8 sm:py-4 bg-cyan-600 text-white rounded-xl hover:bg-cyan-700 transition-all shadow-lg hover:shadow-xl font-bold text-base sm:text-lg"
          >
        ...
        {/* Sessions List */}
        {sessions.length === 0 ? (
          <div className="bg-white rounded-xl shadow-md border-2 border-cyan-200 p-8 sm:p-12 text-center">
            <GiShuttlecock className="text-5xl sm:text-6xl text-gray-300 mx-auto mb-3 sm:mb-4" />
            <p className="text-gray-500 text-base sm:text-lg mb-4 sm:mb-6">
              아직 집계 내역이 없습니다.
            </p>
            <button
              onClick={() => setShowSetup(true)}
              className="px-5 py-2.5 sm:px-6 sm:py-3 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 transition-colors font-medium text-sm sm:text-base"
            >
              첫 집계 시작하기
            </button>
          </div>
              ...
              {sessions
                ...
                .map((session) => (
                  <div
                    ...
                    className="bg-white rounded-xl shadow-md border-2 border-gray-200 hover:border-cyan-500 hover:shadow-lg transition-all cursor-pointer p-4 sm:p-6"
                  >
                    <div className="flex items-start justify-between mb-2 sm:mb-3">
                      <div className="flex-1 min-w-0">
                        {editingSessionId === session.id ? (
                          <div className="flex gap-1 sm:gap-2 mb-1" onClick={(e) => e.stopPropagation()}>
                            <input
                              type="text"
                              value={editingSessionName}
                              onChange={(e) => setEditingSessionName(e.target.value)}
                              className="flex-1 px-2 py-1 border-2 border-cyan-500 rounded text-xs sm:text-sm font-bold"
                              autoFocus
                            />
                            <button
                              onClick={(e) => handleSaveSessionName(session.id, e)}
                              className="px-1.5 sm:px-2 py-1 bg-cyan-600 text-white rounded text-[10px] sm:text-xs hover:bg-cyan-700"
                            >
                              저장
                            </button>
                            <button
                              onClick={handleCancelEdit}
                              className="px-1.5 sm:px-2 py-1 bg-gray-500 text-white rounded text-[10px] sm:text-xs hover:bg-gray-600"
                            >
                              취소
                            </button>
                          </div>
                          ...
                          <div className="flex items-center gap-1.5 sm:gap-2 mb-1">
                            ...
                            <button
                              ...
                              className="text-gray-500 hover:text-cyan-600 p-0.5 sm:p-1"
                              title="이름 수정"
                            >
                      ...
                      <button
                        ...
                        className="text-red-600 hover:text-red-800 p-1.5 sm:p-2 hover:bg-red-50 rounded transition-colors flex-shrink-0"
                        title="삭제"
                      >
                    ...
                    {session.stagedMatches.length > 0 && (
                      <div className="mt-2 px-2 py-0.5 sm:py-1 bg-cyan-50 rounded text-[10px] sm:text-xs text-cyan-700 font-medium">
